...
def main(resample_dir, raw_data_dir, output_dir, clip_len, time_step_size, is_fft=True):
    with open(os.path.join(FILE_MARKER_DIR, "trainSet_seq2seq_" + str(clip_len) + "s_sz.txt"), "r") as f:
        train_str = f.readlines()
    train_tuples = [curr_str.strip('\n').split(',') for curr_str in train_str]
    with open(os.path.join(FILE_MARKER_DIR, "trainSet_seq2seq_" + str(clip_len) + "s_nosz.txt"), "r") as f:
        train_str = f.readlines()
    train_tuples = train_tuples + \
        [curr_str.strip('\n').split(',') for curr_str in train_str]

    with open(os.path.join(FILE_MARKER_DIR, "devSet_seq2seq_" + str(clip_len) + "s_sz.txt"), "r") as f:
        dev_str = f.readlines()
    dev_tuples = [curr_str.strip('\n').split(',') for curr_str in dev_str]
    with open(os.path.join(FILE_MARKER_DIR, "devSet_seq2seq_" + str(clip_len) + "s_nosz.txt"), "r") as f:
        dev_str = f.readlines()
    dev_tuples = dev_tuples + \
        [curr_str.strip('\n').split(',') for curr_str in dev_str]

    with open(os.path.join(FILE_MARKER_DIR, "testSet_seq2seq_" + str(clip_len) + "s_sz.txt"), "r") as f:
        test_str = f.readlines()
    test_tuples = [curr_str.strip('\n').split(',') for curr_str in test_str]
    with open(os.path.join(FILE_MARKER_DIR, "testSet_seq2seq_" + str(clip_len) + "s_nosz.txt"), "r") as f:
        test_str = f.readlines()
    ...
    output_dir = os.path.join(output_dir, 'clipLen' + str(clip_len) + '_timeStepSize' + str(time_step_size))
    ...
    for idx in tqdm(range(len(all_tuples))):
        ...
        eeg_clip, _ = computeSliceMatrix(h5_fn=os.path.join(resample_dir, h5_fn),
                                         edf_fn=edf_fn_full,
                                         clip_idx=clip_idx,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--resampled_dir", type=str, default=None, help="Directory to resampled signals.")
    parser.add_argument("--raw_data_dir", type=str, default=None, help="Directory to raw edf files.")
    ...
    parser.add_argument("--is_fft", action="store_true", default=False, help="Whether to perform FFT.")

    args = parser.parse_args()
    main(args.resampled_dir, args.raw_data_dir, args.output_dir, args.clip_len, args.time_step_size, args.is_fft)
