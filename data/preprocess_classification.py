...
def main(resample_dir, raw_data_dir, output_dir, clip_len, time_step_size, is_fft=True):
    with open(os.path.join(FILE_MARKER_DIR, "trainSet_seizure_files.txt"), "r") as f:
        train_str = f.readlines()
    train_tuples = [curr_str.strip('\n').split(',') for curr_str in train_str]

    with open(os.path.join(FILE_MARKER_DIR, "devSet_seizure_files.txt"), "r") as f:
        dev_str = f.readlines()
    dev_tuples = [curr_str.strip('\n').split(',') for curr_str in dev_str]

    with open(os.path.join(FILE_MARKER_DIR, "testSet_seizure_files.txt"), "r") as f:
    ...
    for idx in tqdm(range(len(all_tuples))):
        ...
        eeg_clip = computeSliceMatrix(h5_fn=os.path.join(resample_dir, h5_fn),
                                      edf_fn=edf_fn_full,
